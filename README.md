...
- Enter Powershell command to test compatability:  
  ```$r=(Get-ItemProperty 'HKLM:\SOFTWARE\Microsoft\NET Framework Setup\NDP\v4\Full' -ErrorAction SilentlyContinue).Release; if($r -ge 528040){"Compatible with RDP autoclicker (.NET 4.8+)"} else{"Not compatible (requires .NET Framework 4.8 or later)"}```
